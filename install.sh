...
on_failure() {
    ...
    if [ -f "./install-troubleshooter.sh" ]; then
        ...
        echo "Could not locate install-troubleshooter.sh in current directory."
    fi
...
case "$OS_TYPE" in
    Linux*)
        if [ -f /etc/os-release ]; then
            ...
            echo "OS detected:      Linux (Unable to parse /etc/os-release)"
        fi
        ...
        case "${ID:-unknown}" in
            ubuntu|debian|fedora|arch|centos|rhel|alpine|pop|manjaro|mint)
                ...
                ;;
            *)
                echo "Warning: Running on an unverified Linux distribution (${DISTRO_NAME}). Proceeding with installation..."
                ;;
        esac
        ;;
...
if [ ! -w "$NPM_PREFIX" ] && [ ! -w "$NPM_GLOBAL_DIR" 2>/dev/null ] && [ "$(id -u)" -ne 0 ]; then
    echo "Warning: Write access denied for global npm directory ($NPM_PREFIX)."
